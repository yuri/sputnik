module(..., package.seeall) --  sputnik.auth.sqlite3

-----------------------------------------------------------------------------
-- Implements Sputnik's authentication API using a SQLite3 database.
-----------------------------------------------------------------------------
      ...
      for index = 1, count do
         ...
         if type == 'string' then
            if value:find("%z") then
               error("sputnik.auth.sqlite3 cannot store embedded zeros")
            end
