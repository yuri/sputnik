...
-- layer, specifically written for the provided MySQL driver.
-----------------------------------------------------------------------------
...
function Storage:new(params, versium)
	...
	if not con then
		versium.storage_error("MySQL Versium: Could not connect to database")
	end

	self.tables = {}

	-- Check to verify that the given tables exist
	...
	for idx,tbl in ipairs(tables) do
		...
		local cmd = prepare("SHOW TABLES LIKE %s;", self.tables[tbl])
